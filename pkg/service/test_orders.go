// // C:\GoProject\src\eShop\pkg\service\test_orders.go
...
func GenerateRandomOrders(count int) error {
	...
	for i := 0; i < count; i++ {
		...
		for j := 0; j < productCount; j++ {
			...
			// Правильное вычисление суммы для элемента заказа
			...
			orderItem := models.OrderItem{
				...
				Price:     product.RetailPrice, // Цена товара
				Total:     total,               // Общая сумма для элемента заказа (Цена * Количество)
			}
